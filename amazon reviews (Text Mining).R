...
############################ Amazon Reviews #############################
aurl <- "https://www.amazon.in/OnePlus-Mirror-Blue-128GB-Storage/product-reviews/B07HGMLBW1/ref=cm_cr_arp_d_paging_btm_next_2?ie=UTF8&reviewerType=all_reviews&pageNumber"
amazon_reviews <- NULL
for (i in 1:20){
...
setwd("D:\\ExcelR\\Text_mining_assignment")
getwd()
write.table(amazon_reviews,"OnePlus.txt",row.names = T)


## Using While loop to get all reviews without using page number ############## 
#### Simple Example ##################################################
samp_url <- "https://www.amazon.in/OnePlus-Mirror-Blue-128GB-Storage/product-reviews/B07HGMLBW1/ref=cm_cr_arp_d_paging_btm_next_2?ie=UTF8&reviewerType=all_reviews&pageNumber"
i=1
p=1
OnePlus <- NULL
while(p>0){
...
letterCloud(w1, word = "oneplus", wordSize = 0.5)
?letterCloud
